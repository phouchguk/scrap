...
const int$to_text=(v)=>String(v);
const float$to_text=(v)=>{if(Number.isInteger(v))return v.toFixed(1);return String(v);};
const text$to_int=(v)=>BigInt(parseInt(v,10));
const text$slice=(start)=>(end)=>(v)=>v.slice(Number(start),Number(end));
const text$at=(idx)=>(v)=>{const i=Number(idx);return i>=0&&i<v.length?{_tag:"just",_val:v[i]}:{_tag:"nothing",_val:null};};
const text$chars=(v)=>v.split("");
const text$contains=(needle)=>(haystack)=>_bool(haystack.includes(needle));
const text$starts_with=(prefix)=>(v)=>_bool(v.startsWith(prefix));
const list$range=(start)=>(end)=>Array.from({length:Math.max(0,Number(end)-Number(start))},(_, i)=>BigInt(Number(start)+i));
